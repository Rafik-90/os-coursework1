...
public abstract class Process implements Comparable<Process> {
  ...
  public int getWaitingTime() {
    ...
    if (this.state != State.TERMINATED) {
      // If the process is not terminated, we cannot accurately calculate the waiting time without the current time.
      // Thus, returning -1 or some indicator of inapplicability might be necessary.
      return -1;
    }

    // Calculate the waiting time as the total time in the system minus the running time and blocked time.
    int totalInSystem = this.terminatedTime - this.createdTime;

      return totalInSystem - this.cpuTime - this.blockedTime;
  }
